...
async function manageTrollMutes() {
  try {
    ...
    for (const trollMute of activeTrollMutes) {
      ...
      console.log(`Time since last cycle: ${now - trollMute.lastCycleTime}ms`);
        ...
        if (timeSinceLastCycle >= trollMute.speakDuration) {
          ...
          try {
            ...
            if (
              ...
              console.log(`Sent mute notification in channel ${channel.name}`);
            }
          } catch (error) {
...
process.on("SIGINT", () => {
  if (client.trollMuteInterval) {
    clearInterval(client.trollMuteInterval);
    console.log("Cleared trollmute interval");
  }
  process.exit(0);
});
...
client.on("guildMemberAdd", async (member) => {
  try {
    ...
    if (activeTrollMute) {
      console.log(`User ${member.user.tag} rejoined with an active trollmute`);
